...
echo "🔥 INICIANDO CORRECCIÓN FINAL (MODELO CONFIG Y PATCH)..."
...
EOF
echo "✅ Archivo corregido: backend/app/models/governance.py"

# ==========================================
# 2. FIX: ENDPOINTS DE CONFIGURACIÓN Y LECTURA DE .ENV
# ==========================================
...
def _format_config(config_obj):
    """Mapea el objeto DB al formato que espera el Frontend, leyendo también el .env"""
    return {
        "id": config_obj.id,
        ...
        # 🔥 FIX: Validamos si la key está en la Base de Datos OR en el archivo .env
        "has_api_key_openai": bool(config_obj.api_key_openai) or bool(settings.openai_api_key),
        "has_api_key_anthropic": bool(config_obj.api_key_anthropic) or bool(settings.anthropic_api_key),
        "has_api_key_gemini": bool(config_obj.api_key_gemini) or bool(settings.gemini_api_key),
    }
...
async def get_config(db: AsyncSession = Depends(get_db)):
    try:
        ...
        if not configs:
            # Fallback por defecto si la tabla está vacía
            return [{
                "id": 1,
        ...
        # Fallback de seguridad en caso de que la tabla aún no se cree
        return [{
            "id": 1,
            "derivar_en_problemas": False,
            "auto_aprobar_ejecucion": False,
...
async def update_config(config_id: int, payload: dict, db: AsyncSession = Depends(get_db)):
    ...
    if not config_obj:
        # Si no existe, lo creamos de forma idempotente para evitar errores 500
...
EOF
echo "✅ Archivo corregido: backend/app/routers/config.py"

echo "🚀 LISTO. Ahora debes reiniciar tu contenedor Backend ('docker compose restart backend') para aplicar los cambios a la Base de Datos."